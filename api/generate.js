...
<head>
    ...
    <!-- PayPal SDK - IMPORTANT: Replace 'YOUR_CLIENT_ID' with your actual PayPal Client ID -->
    ...
    <style>
        ...
        .scroller__inner {
            display: flex; gap: 1rem; padding-block: 1rem; width: max-content;
            animation: scroll 60s linear infinite;
        }
        .scroller__inner img {
            height: 180px; width: auto; border-radius: 0.75rem; object-fit: cover;
        }
...
<body class="bg-gray-900 text-white">
        ...
        <section class="py-16 px-6 max-w-7xl mx-auto">
             <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-gray-800 rounded-2xl p-8 border border-gray-700 flex flex-col h-full">
                    <div class="flex-grow"><div class="w-12 h-12 bg-indigo-600/20 rounded-full flex items-center justify-center mb-4"><svg class="w-6 h-6 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg></div><h3 class="text-2xl font-bold mb-2">Launch Ads Directly to Meta</h3><p class="text-gray-400 mb-6">Now, with just a single click, you can launch ads directly from Projects to Meta, saving time and simplifying the process.</p></div><img src="https://i.imgur.com/RzmcLQd.png" alt="Ad example for Meta launch" class="rounded-xl object-cover w-full h-64 mt-auto"></div>
                <div class="bg-gray-800 rounded-2xl p-8 border border-gray-700 flex flex-col h-full">
                    <div class="flex-grow"><div class="w-12 h-12 bg-pink-600/20 rounded-full flex items-center justify-center mb-4"><svg class="w-6 h-6 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg></div><h3 class="text-2xl font-bold mb-2">Monitor Ad Performance & Health</h3><p class="text-gray-400 mb-6">Easily monitor ad performance with the new Ad Level View in My Ads. Track key metrics, adjust ad status, and check ad health—all in one place.</p></div><img src="https://i.imgur.com/7xL8pXD.png" alt="Ad performance dashboard" class="rounded-xl object-cover w-full h-64 mt-auto"></div>
            </div>
        </section>
    ...
    <script>
        ...
        const SECURE_API_ENDPOINT = 'https://campaign-generator-mpnygq511-luchille-brown-s-projects.vercel.app/api/generate';
        ...
        async function handleGenerateCampaign() {
            ...
            if ((!productName && !campaignImageBase64) || platforms.length === 0) return showMessageBox('Please provide a product name or image and select platforms.', true);

            generateCampaignBtn.disabled = true;
            generateCampaignBtn.textContent = 'Generating...';
            campaignOutput.innerHTML = `<div class="col-span-2 text-center p-4 text-gray-400">Generating campaign, please wait...</div>`;

            try {
                const basePrompt = `Create a social media campaign for the following platforms: ${platforms.join(', ')}. For each platform, provide a "headline", "ad_copy", "visual_concept", and "call_to_action". Focus on highlighting the scientific and botanical benefits of the product. Format as a single JSON array of objects. Each object should have keys "platform_name", "headline", "ad_copy", "visual_concept", "call_to_action".`;
                ...
                if (campaignImageBase64) {
                    const descResult = await callSecureApi('productAnalysis', { contents: [{ parts: [{ text: "Describe this product." }, { inlineData: { mimeType: "image/jpeg", data: campaignImageBase64 } }] }] });
                    const pDesc = descResult.candidates[0].content.parts[0].text;
                    const ePN = productName || pDesc;
                    campaignTextPayload.contents[0].parts.push({ text: `Based on the image of a ${ePN}, ${basePrompt}` }, { inlineData: { mimeType: "image/jpeg", data: campaignImageBase64 } });
                } else {
                    campaignTextPayload.contents[0].parts.push({ text: `Create a campaign for a product called "${productName}". ${basePrompt}` });
                }
                ...
                if (!campaignData) throw new Error('Failed to get campaign data.');
                ...
                for (const platformData of campaignData) {
                    ...
                    generateAndDisplayVisual(platformData, productName);
                }
            } catch (error) {
                console.error("Campaign Generation Error:", error);
                campaignOutput.innerHTML = `<div class="col-span-2 text-center text-red-500">Failed to generate campaign: ${error.message}</div>`;
                showMessageBox(error.message, true);
            } finally {
        ...
        function createCampaignCard(platformData) {
            ...
            card.innerHTML = `<h3 class="text-xl font-bold">${platformData.platform_name}</h3><p class="text-sm text-gray-400 mt-2"><strong>Headline:</strong> ${platformData.headline || 'N/A'}</p><p class="text-sm text-gray-400"><strong>Ad Copy:</strong> ${platformData.ad_copy || 'N/A'}</p><p class="text-sm text-gray-400"><strong>Visual Concept:</strong> ${platformData.visual_concept || 'N/A'}</p><p class="text-sm text-gray-400"><strong>Call to Action:</strong> ${platformData.call_to_action || 'N/A'}</p><div class="mt-4 aspect-video bg-gray-700 rounded-lg flex items-center justify-center"><img id="img-${platform}" class="hidden w-full h-full object-cover rounded-lg" /><div id="loader-${platform}" class="text-gray-400">Loading visual...</div></div><div id="controls-${platform}" class="mt-4 flex space-x-2 hidden"><button class="download-visual-btn flex-1 bg-green-600 text-white text-sm py-2 px-3 rounded-lg hover:bg-green-700 transition-colors">Download</button><button class="use-in-ad-gen-btn flex-1 bg-blue-600 text-white text-sm py-2 px-3 rounded-lg hover:bg-blue-700 transition-colors" data-platform="${platform}" data-concept="${platformData.visual_concept || ''}">Use in Ad Generator</button></div>`;
            return card;
        }
        ...
        async function generateAndDisplayVisual(platformData, productName) {
            ...
            const controlsEl = document.getElementById(`controls-${platform}`);
            if (campaignImageBase64) {
                 imgEl.src = `data:image/jpeg;base64,${campaignImageBase64}`;
                 imgEl.classList.remove('hidden');
                 loaderEl.classList.add('hidden');
                 controlsEl.classList.remove('hidden');
            } else if (platformData.visual_concept) {
                try {
                    const visualResult = await callSecureApi('campaignVisual', { instances: [{ prompt: `Marketing photo: ${platformData.visual_concept} for ${productName}.` }], parameters: { "sampleCount": 1 } });
                ...
                loaderEl.textContent = 'No visual concept.';
            }
        }

        function setupEventListeners() {
            const { generateAdBtn, resetBtn, productImageUpload, campaignImageUpload, downloadBtn, generateCampaignBtn, launchToMetaBtn, connectAndLaunchBtn, showAdGeneratorBtn, showCampaignMakerBtn, showDashboardBtn, addNewAdCard, addNewAdBtn, signupForm, copyAdBtn, campaignOutput } = DOMElements;
            ...
            connectAndLaunchBtn.addEventListener('click', () => {
                const metaId = DOMElements.metaAdAccountIdInput.value.trim();
                if (!metaId) return showMessageBox('Please enter a valid Meta Ad Account ID.', true);
                handleLaunchToMeta(metaId);
            });

            const tabs = { ad: [DOMElements.adGenerator, showAdGeneratorBtn], campaign: [DOMElements.campaignMaker, showCampaignMakerBtn], dashboard: [DOMElements.myAdsDashboardSection, showDashboardBtn] };
            ...
            showAdGeneratorBtn.addEventListener('click', () => switchTab('ad'));
            showCampaignMakerBtn.addEventListener('click', () => switchTab('campaign'));
            showDashboardBtn.addEventListener('click', () => switchTab('dashboard'));

            addNewAdCard.addEventListener('click', () => switchTab('ad'));
            addNewAdBtn.addEventListener('click', () => switchTab('ad'));
            ...
            signupForm.addEventListener('submit', (e) => {
                ...
                if(email) {
                    isTrial = false;
                    closeSignupModal();
                    showMessageBox(`Welcome! Your account for ${email} is active. You can now download and launch ads.`);
                    DOMElements.downloadBtn.classList.remove('cursor-not-allowed', 'opacity-50');
            ...
            copyAdBtn.addEventListener('click', () => {
                navigator.clipboard.writeText(DOMElements.adCopyText.textContent);
                showMessageBox('Ad copy copied to clipboard!');
            });
            
            campaignOutput.addEventListener('click', (event) => {
                if (event.target.closest('.download-visual-btn')) handleDownloadVisual(event);
                if (event.target.closest('.use-in-ad-gen-btn')) handleUseInAdGenerator(event);
            });
        }
        ...
        async function handleImageUpload(event) {
            ...
            if (file) {
                ...
                DOMElements.uploadLabel.classList.add('bg-green-700');
        ...
        async function handleCampaignImageUpload(event) {
            ...
            if (file) {
                ...
                DOMElements.campaignUploadLabel.classList.add('bg-green-700');
        ...
        function handleDownloadAd() {
            if (isTrial) {
                if (trialDownloads >= TRIAL_DOWNLOAD_LIMIT) {
                    showMessageBox('You have reached your free trial download limit.', true);
                    openSignupModal();
                    return;
                }
                trialDownloads++;
                showMessageBox(`Download successful! You have ${TRIAL_DOWNLOAD_LIMIT - trialDownloads} free downloads remaining.`);
            }
            if (!finalAdImageBase64) return showMessageBox('Please generate an ad first.', true);
            downloadBase64Image(`data:image/png;base64,${finalAdImageBase64}`, 'soul-tribe-ad.png');
        }
        ...
        function handleDownloadVisual(event) {
            ...
            if (imgElement && imgElement.src.startsWith('data:')) {
                downloadBase64Image(imgElement.src, `${platformName}-visual.png`);
            } else {
                showMessageBox('Image is not available for download.', true);
            }
        }

        function handleLaunchToMeta(metaId) {
            const { connectAndLaunchBtn, adsDashboardGrid, adCopyText, showDashboardBtn } = DOMElements;
            ...
            setTimeout(() => {
                ...
                showMessageBox(`Campaign launched successfully to Meta account ${metaId}!`);
                const newAdCard = document.createElement('div');
                newAdCard.className = 'bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden';
                let headline = adCopyText.textContent.split("Headline:")[1]?.split("Body:")[0]?.trim() || "Newly Launched Ad";
                newAdCard.innerHTML = `<img src="data:image/png;base64,${finalAdImageBase64}" alt="Ad Preview" class="w-full h-64 object-cover"><div class="p-6"><h3 class="font-semibold text-white mb-2">"${headline}"</h3><p class="text-sm text-gray-400 mb-4">Status: <span class="font-semibold text-yellow-400">In Review</span></p><div class="grid grid-cols-3 gap-4 text-center"><div><p class="text-xs text-gray-400">Reach</p><p class="text-lg font-bold text-white">0</p></div><div><p class="text-xs text-gray-400">Clicks</p><p class="text-lg font-bold text-white">0</p></div><div><p class="text-xs text-gray-400">CTR</p><p class="text-lg font-bold text-white">0%</p></div></div><button class="mt-4 w-full bg-gray-700 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-600 transition-colors">View Performance</button></div>`;
                adsDashboardGrid.insertBefore(newAdCard, DOMElements.addNewAdCard);
        ...
        function handleReset() {
            const { backgroundPromptInput, uploadLabel, productImageUpload, adImage, adCopyContainer, canvasControls, launchToMetaContainer, overlay, overlayText } = DOMElements;
            backgroundPromptInput.value = '';
            
            // Create a new file input to truly reset it
            const newUploadInput = productImageUpload.cloneNode(true);
            productImageUpload.parentNode.replaceChild(newUploadInput, productImageUpload);
            DOMElements.productImageUpload = newUploadInput; // Update reference
            newUploadInput.addEventListener('change', handleImageUpload);

            uploadLabel.textContent = 'Upload Product Image';
            uploadLabel.classList.remove('bg-green-700');
        ...
        function handleUseInAdGenerator(event) {
            const { platform, concept } = event.target.closest('button').dataset;
            const imgElement = document.getElementById(`img-${platform}`);
            if (imgElement && imgElement.src) {
                DOMElements.showAdGeneratorBtn.click();
                DOMElements.backgroundPromptInput.value = concept;
                productImageBase64 = imgElement.src.split(',')[1];
                DOMElements.adImage.src = imgElement.src;
                DOMElements.uploadLabel.textContent = `Using image from ${platform} campaign`;
                DOMElements.uploadLabel.classList.add('bg-green-700');
                DOMElements.overlay.classList.add('hidden');
                showMessageBox(`Loaded visual concept from ${platform} into Ad Generator.`);
                DOMElements.adGenerator.scrollIntoView({ behavior: 'smooth' });
            } else {
                showMessageBox('Could not find the image to use.', true);
            }
        }
        
        function setupPayPalButtons() {
            const plans = {
                starter: { container: '#paypal-starter-button-container', value: '19.00' },
                pro: { container: '#paypal-pro-button-container', value: '49.00' },
                business: { container: '#paypal-business-button-container', value: '99.00' }
            };
                ...
                try {
                    paypal.Buttons({
                        createOrder: function(data, actions) {
                            return actions.order.create({ purchase_units: [{ amount: { value: plans[plan].value } }] });
                        },
                        onApprove: function(data, actions) {
                            return actions.order.capture().then(function(details) {
                                isTrial = false;
                                showMessageBox('Payment successful! Welcome, ' + details.payer.name.given_name);
                                document.querySelector(plans[plan].container).innerHTML = `<div class="text-center text-green-400 font-bold p-3 bg-green-900/50 rounded-lg">Current Plan</div>`;
                                Object.values(plans).forEach(p => { if(p.container !== plans[plan].container) document.querySelector(p.container).innerHTML = ''; });
                            });
                        },
                    ...
                    }).render(plans[plan].container);
                } catch(e) {
                    console.error("PayPal button rendering error:", e);
                    document.querySelector(plans[plan].container).innerHTML = `<div class="text-center text-red-400 text-sm">Could not load payment options. Please check your PayPal Client ID.</div>`;
                }
            }
        }
